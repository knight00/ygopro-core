...
int32 scriptlib::card_get_linked_zone(lua_State* L) {
	...
	auto cp = lua_get<uint8>(L, 2, pcard->current.controler);
...
int32 scriptlib::card_get_column_zone(lua_State* L) {
	...
	auto left = lua_get<uint8, 0>(L, 3);
	auto right = lua_get<uint8, 0>(L, 4);
	auto cp = lua_get<uint8>(L, 5, pcard->current.controler);
...
int32 scriptlib::card_is_attack(lua_State* L) {
	...
	if(!(pcard->data.type & TYPE_MONSTER) && !(pcard->get_type() & TYPE_MONSTER) && !pcard->is_affected_by_effect(EFFECT_PRE_MONSTER) && !(((pcard->current.location & LOCATION_MZONE) && !pcard->is_affected_by_effect(EFFECT_SANCT_MZONE)) || ((pcard->current.location & LOCATION_SZONE) && pcard->is_affected_by_effect(EFFECT_ORICA_SZONE))))
	//////kdiy/////////
		lua_pushboolean(L, 0);
	else
...
int32 scriptlib::card_is_defense(lua_State* L) {
	...
	if((pcard->data.type & TYPE_LINK) || !(pcard->data.type & TYPE_MONSTER) && !(pcard->get_type() & TYPE_MONSTER) && !pcard->is_affected_by_effect(EFFECT_PRE_MONSTER) && !(((pcard->current.location & LOCATION_MZONE) && !pcard->is_affected_by_effect(EFFECT_SANCT_MZONE)) || ((pcard->current.location & LOCATION_SZONE) && pcard->is_affected_by_effect(EFFECT_ORICA_SZONE))))
	//////kdiy/////////
		lua_pushboolean(L, 0);
	else
...
int32 scriptlib::card_is_has_effect(lua_State* L) {
	...
	auto check_player = lua_get<uint8, PLAYER_NONE>(L, 3);
	for(const auto& peff : eset) {
...
int32 scriptlib::card_register_flag_effect(lua_State* L) {
	...
	auto desc = lua_get<uint64, 0>(L, 7);
	if(count == 0)
		count = 1;
...
int32 scriptlib::card_copy_effect(lua_State* L) {
	...
	auto code = lua_get<uint32>(L, 2);
	auto reset = lua_get<uint32>(L, 3);
	auto count = lua_get<uint8, 1>(L, 4);
	if(count == 0)
		count = 1;
	if(reset & RESET_PHASE && !(reset & (RESET_SELF_TURN | RESET_OPPO_TURN)))
		reset |= (RESET_SELF_TURN | RESET_OPPO_TURN);
...
int32 scriptlib::card_is_fusion_summonable_card(lua_State* L) {
	check_param_count(L, 1);
	auto pcard = lua_get<card*, true>(L, 1);
	auto summon_type = lua_get<uint32, 0>(L, 2);
...
int32 scriptlib::card_is_special_summonable(lua_State* L) {
	...
	const auto pduel = lua_get<duel*>(L);
	auto pcard = lua_get<card*, true>(L, 1);
	auto sumtype = lua_get<uint32, 0>(L, 2);
...
inline int32 spsummonable_rule(lua_State* L, uint32 cardtype, uint32 sumtype, uint32 offset) {
	...
	auto minc = lua_get<uint16, 0>(L, 4 + offset);
	auto maxc = lua_get<uint16, 0>(L, 5 + offset);
...
int32 scriptlib::card_is_can_be_special_summoned(lua_State* L) {
	...
	auto sumpos = lua_get<uint8, POS_FACEUP>(L, 7);
	auto toplayer = lua_get<uint8>(L, 8, sumplayer);
	auto zone = lua_get<uint32, 0xff>(L, 9);
...
int32 scriptlib::card_is_able_to_remove(lua_State* L) {
	...
	const auto pduel = lua_get<duel*>(L);
	auto pcard = lua_get<card*, true>(L, 1);
	auto p = lua_get<uint8>(L, 2, pduel->game_field->core.reason_player);
	auto pos = lua_get<uint8, POS_FACEUP>(L, 3);
	auto reason = lua_get<uint32, REASON_EFFECT>(L, 4);
...
int32 scriptlib::card_is_able_to_remove_as_cost(lua_State* L) {
	...
	const auto pduel = lua_get<duel*>(L);
	auto pcard = lua_get<card*, true>(L, 1);
	auto pos = lua_get<uint8, POS_FACEUP>(L, 2);
...
int32 scriptlib::card_is_discardable(lua_State* L) {
	...
	auto reason = lua_get<uint32, REASON_COST>(L, 2);
...
int32 scriptlib::card_is_level_below(lua_State* L) {
	...
	bool lv = true;
	if((pcard->get_type() & (TYPE_XYZ)) && !(pcard->get_type() & (TYPE_LINK)) && !(pcard->is_affected_by_effect(EFFECT_LEVEL_RANK) || pcard->is_affected_by_effect(EFFECT_LEVEL_RANK_S) || pcard->is_affected_by_effect(EFFECT_RANK_LEVEL) || pcard->is_affected_by_effect(EFFECT_RANK_LEVEL_S)))
	    lv = false;
	if((pcard->get_type() & (TYPE_LINK)) && !((pcard->is_affected_by_effect(EFFECT_LEVEL_RANK) || pcard->is_affected_by_effect(EFFECT_LEVEL_RANK_S) || pcard->is_affected_by_effect(EFFECT_RANK_LEVEL) || pcard->is_affected_by_effect(EFFECT_RANK_LEVEL_S)) && (pcard->is_affected_by_effect(EFFECT_LINK_RANK) || pcard->is_affected_by_effect(EFFECT_LINK_RANK_S))))
	    lv = false;		
...
int32 scriptlib::card_is_rank_below(lua_State* L) {
	...
	if(!(pcard->get_type() & (TYPE_XYZ)) && !(pcard->get_type() & (TYPE_LINK)) && !(pcard->is_affected_by_effect(EFFECT_LEVEL_RANK) || pcard->is_affected_by_effect(EFFECT_LEVEL_RANK_S) || pcard->is_affected_by_effect(EFFECT_RANK_LEVEL) || pcard->is_affected_by_effect(EFFECT_RANK_LEVEL_S)))
	    lv = false;
	if((pcard->get_type() & (TYPE_LINK)) && !(pcard->is_affected_by_effect(EFFECT_LINK_RANK) || pcard->is_affected_by_effect(EFFECT_LINK_RANK_S)))
	    lv = false;			
...
int32 scriptlib::card_is_link_below(lua_State* L) {
	...
	bool lv = true;
	if((pcard->get_type() & (TYPE_XYZ)) && !(pcard->get_type() & (TYPE_LINK)) && !(pcard->is_affected_by_effect(EFFECT_LINK_RANK) || pcard->is_affected_by_effect(EFFECT_LINK_RANK_S)))
	    lv = false;
	if(!(pcard->get_type() & (TYPE_XYZ)) && !(pcard->get_type() & (TYPE_LINK)) && !((pcard->is_affected_by_effect(EFFECT_LEVEL_RANK) || pcard->is_affected_by_effect(EFFECT_LEVEL_RANK_S) || pcard->is_affected_by_effect(EFFECT_RANK_LEVEL) || pcard->is_affected_by_effect(EFFECT_RANK_LEVEL_S)) && (pcard->is_affected_by_effect(EFFECT_LINK_RANK) || pcard->is_affected_by_effect(EFFECT_LINK_RANK_S))))
	    lv = false;			
...
int32 scriptlib::card_is_can_add_counter(lua_State* L) {
	...
	auto pcard = lua_get<card*, true>(L, 1);
	auto countertype = lua_get<uint16>(L, 2);
	auto count = lua_get<uint16, 0>(L, 3);
	bool singly = lua_get<bool, false>(L, 4);
	auto loc = lua_get<uint16, 0>(L, 5);
...
int32 scriptlib::card_is_can_overlay(lua_State* L) {
	...
	const auto pduel = lua_get<duel*>(L);
	auto pcard = lua_get<card*, true>(L, 1);
	auto playerid = lua_get<uint8>(L, 2, pduel->game_field->core.reason_player);
...
int32 scriptlib::card_check_unique_onfield(lua_State* L) {
	...
	auto check_location = lua_get<uint8, LOCATION_ONFIELD>(L, 3) & LOCATION_ONFIELD;
