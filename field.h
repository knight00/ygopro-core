...
public:
	...
	card* get_field_card(uint8 playerid, uint32 location, uint32 sequence);
	int32 is_location_useable(uint8 playerid, uint32 location, uint32 sequence);
	...
	int32 get_tofield_count(card* pcard, uint8 playerid, uint8 location, uint8 uplayer, uint32 reason, uint32 zone = 0xff, uint32* list = 0);
	...
	int32 get_forced_zones(card* pcard, uint8 playerid, uint8 location, uint8 uplayer, uint32 reason);
	uint32 get_rule_zone_fromex(int8 playerid, card* pcard);
	uint32 get_linked_zone(int8 playerid, bool free = false);
	void get_linked_cards(uint8 self, uint8 location1, uint8 location2, card_set* cset);
	int32 check_extra_link(int8 playerid, card* pcard, int32 sequence);
	void get_cards_in_zone(card_set* cset, uint32 zone, int8 playerid, int32 location);
	...
	uint8 get_pzone_index(uint8 seq);
	...
	void reset_chain();
	void add_effect_code(uint32 code, uint32 flag, uint8 playerid);
	uint32 get_effect_code(uint32 code, uint32 flag, uint8 playerid);
	void dec_effect_code(uint32 code, uint32 flag, uint8 playerid);
	...
	void check_card_counter(card* pcard, int32 counter_type, int8 playerid);
	void check_card_counter(group* pgroup, int32 counter_type, int8 playerid);
	void check_chain_counter(effect* peffect, int8 playerid, int32 chainid, bool cancel = false);
	void set_spsummon_counter(uint8 playerid, bool add = true, bool chain = false);
	int32 check_spsummon_counter(uint8 playerid, uint16 ct = 1);
	...
	int32 is_player_can_spsummon_count(uint8 playerid, uint16 count);
	...
	int32 get_cteffect(effect* peffect, int8 playerid, int32 store);
	int32 get_cteffect_evt(effect* feffect, int8 playerid, const tevent& e, int32 store);
	...
	void get_control(card_set* targets, effect* reason_effect, uint8 reason_player, uint8 playerid, uint32 reset_phase, uint32 reset_count, uint32 zone);
	void get_control(card* target, effect* reason_effect, uint8 reason_player, uint8 playerid, uint32 reset_phase, uint32 reset_count, uint32 zone);
	void swap_control(effect* reason_effect, uint8 reason_player, card_set* targets1, card_set* targets2, uint32 reset_phase, uint32 reset_count);
	void swap_control(effect* reason_effect, uint8 reason_player, card* pcard1, card* pcard2, uint32 reset_phase, uint32 reset_count);
	void equip(uint32 equip_player, card* equip_card, card* target, uint32 up, uint32 is_step);
	void draw(effect* reason_effect, uint32 reason, uint8 reason_player, uint8 playerid, uint32 count);
	void damage(effect* reason_effect, uint32 reason, uint8 reason_player, card* reason_card, uint8 playerid, uint32 amount, uint32 is_step = FALSE);
	void recover(effect* reason_effect, uint32 reason, uint8 reason_player, uint8 playerid, uint32 amount, uint32 is_step = FALSE);
	void summon(uint8 sumplayer, card* target, effect* proc, uint32 ignore_count, uint32 min_tribute, uint32 zone = 0x1f);
	void mset(uint32 setplayer, card* target, effect* proc, uint32 ignore_count, uint32 min_tribute, uint32 zone = 0x1f);
	void special_summon_rule(uint8 sumplayer, card* target, uint32 summon_type);
	void special_summon(card_set* target, uint32 sumtype, uint8 sumplayer, uint8 playerid, uint32 nocheck, uint32 nolimit, uint32 positions, uint32 zone);
	void special_summon_step(card* target, uint32 sumtype, uint8 sumplayer, uint8 playerid, uint32 nocheck, uint32 nolimit, uint32 positions, uint32 zone);
	void special_summon_complete(effect* reason_effect, uint8 reason_player);
	void destroy(card_set* targets, effect* reason_effect, uint32 reason, uint8 reason_player, uint8 playerid = 2, uint32 destination = 0, uint32 sequence = 0);
	void destroy(card* target, effect* reason_effect, uint32 reason, uint8 reason_player, uint8 playerid = 2, uint32 destination = 0, uint32 sequence = 0);
	void release(card_set* targets, effect* reason_effect, uint32 reason, uint8 reason_player);
	void release(card* target, effect* reason_effect, uint32 reason, uint8 reason_player);
	void send_to(card_set* targets, effect* reason_effect, uint32 reason, uint8 reason_player, uint8 playerid, uint32 destination, uint32 sequence, uint32 position, uint32 ignore = false);
	void send_to(card* target, effect* reason_effect, uint32 reason, uint8 reason_player, uint8 playerid, uint32 destination, uint32 sequence, uint32 position, uint32 ignore = false);
	void move_to_field(card* target, uint32 move_player, uint8 playerid, uint32 destination, uint32 positions, uint32 enable = FALSE, uint32 ret = 0, uint32 is_equip = FALSE, uint32 zone = 0xff, uint32 rule = FALSE, uint32 reason = 0);
	void change_position(card_set* targets, effect* reason_effect, uint8 reason_player, uint32 au, uint32 ad, uint32 du, uint32 dd, uint32 flag, uint32 enable = FALSE);
	void change_position(card* target, effect* reason_effect, uint8 reason_player, uint32 npos, uint32 flag, uint32 enable = FALSE);
	void operation_replace(int32 type, uint16 step, group* targets);
	...
	int32 self_destroy(uint16 step, card* ucard, int8 playerid);
	...
	int32 move_to_field(uint16 step, card* target, uint32 enable, uint32 ret, uint32 is_equip, uint32 zone, uint8 pzone = FALSE, uint32 rule = FALSE, uint32 reason = 0);
