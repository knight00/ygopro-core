...
int32 field::select_battle_command(uint16 step, uint8 playerid) {
	if(step == 0) {
#ifdef BUILD_WITH_AI
		if((player[playerid].isAI) && (core.duel_options & DUEL_SIMPLE_AI)) {
			...
			if(command == 1) {
				...
				returns.at<int8>(0) = 1;
				returns.at<int8>(1) = 0;
				returns.at<int8>(2) = attackerIndex - 1;
			} else if(command == 2) {
	...
	if(step == 0) {
		...
		if((player[playerid].isAI) && (core.duel_options & DUEL_SIMPLE_AI)) {
			...
			if(command > -1) {
				...
				returns.at<int32>(0) = command;
				returns.at<int8>(2) = index - 1;
...
int32 field::select_option(uint16 step, uint8 playerid) {
	if(step == 0) {
		...
		if((player[playerid].isAI) && (core.duel_options & DUEL_SIMPLE_AI)) {
			...
			lua_newtable(L);
			std::vector<uint64> opt = core.select_options;
	...
	if(step == 0) {
		...
		if((player[playerid].isAI) && (core.duel_options & DUEL_SIMPLE_AI)) {
			...
			if(len < min || len > max) {
				...
				if(len == 1) {
					//returns.at<int8>(0) = len; //set number of tagets
					lua_pushinteger(L, 1);
					lua_gettable(L, -2);
					...
					return_cards.list.push_back(core.select_cards[temp - 1]);
					lua_pop(L, 1);
					lua_pop(L, 1);//pop the table off the stack
				} else {
					//returns.at<int8>(0) = len; //set number of tagets
					for(int i = 0; i < len; i++) {
						...
						return_cards.list.push_back(core.select_cards[temp - 1]);
						//returns.at<int8>(i + 1) = temp - 1; //set target index
						lua_pop(L, 1);
					}
					lua_pop(L, 1);//pop the table off the stack
				}
...
#ifdef BUILD_WITH_AI
		if((player[playerid].isAI) && (core.duel_options & DUEL_SIMPLE_AI)) {
			return field::select_card(step, playerid, cancelable, 1, 1);
		} else
#endif //BUILD_WITH_AI
		if ((playerid == 1) && is_flag(DUEL_SIMPLE_AI)) {
			if(cancelable)
	...
	if(step == 0) {
		...
		if((player[playerid].isAI) && (core.duel_options & DUEL_SIMPLE_AI)) {
			...
			if(shouldChain > 0) {
				...
				returns.at<int32>(0) = 0;
				returns.at<int8>(0) = chainIndex - 1;
			} else {
				if(forced > 0) {
					returns.at<int32>(0) = 0;
					returns.at<int8>(0) = 0;
...
#ifdef BUILD_WITH_AI
int32 field::select_place(uint16 step, uint8 playerid, uint32 flag, uint8 count, card* reason_card) {
#else
int32 field::select_place(uint16 step, uint8 playerid, uint32 flag, uint8 count) {
	...
	if(step == 0) {
		...
		if((player[playerid].isAI) && (core.duel_options & DUEL_SIMPLE_AI)) {
			lua_State* L = pduel->lua->lua_state;
			lua_getglobal(L, "OnSelectPlace");
			lua_pushnumber(L, ~flag);
			lua_pushnumber(L, playerid);
			lua_pushnumber(L, count);
			if(reason_card) {
				set_card_to_lua_without_index(L, reason_card, interpreter::get_duel_info(L));
			} else {
				lua_pushboolean(L, false); //set a false
			}
			uint32 result = 0;
			...
			for(int i = 0; i < 7; ++i, filter <<= 1) {
				if(result & filter) {
					returns.at<int8>(p) = playerid;
					returns.at<int8>(p + 1) = LOCATION_MZONE;
					returns.at<int8>(p + 2) = i;
					p += 3;
				}
			}
			filter = 0x100;
			for(int i = 0; i < 8; ++i, filter <<= 1) {
				if(result & filter) {
					returns.at<int8>(p) = playerid;
					returns.at<int8>(p + 1) = LOCATION_SZONE;
					returns.at<int8>(p + 2) = i;
					p += 3;
				}
			}
...
int32 field::select_with_sum_limit(int16 step, uint8 playerid, int32 acc, int32 min, int32 max) {
	if(step == 0) {
		...
		if((player[playerid].isAI) && (core.duel_options & DUEL_SIMPLE_AI)) {
			...
			int len = luaL_len(L, -1);
			/*returns.at<int32>(0) = 1;//return TRUE
			returns.at<int8>(0) = (int8)(len + mustSize); // size
		...
		if((player[playerid].isAI) && (core.duel_options & DUEL_SIMPLE_AI)) {
			...
			if(!core.select_cards.empty()) {
				...
				for(int i = 0; i < len; i++) {
					...
					lua_gettable(L, -2);
					int temp = lua_tonumber(L, -1);
					returns.at<int8>(i) = temp - 1; //set tribute index
					lua_pop(L, 1);
				}
				lua_pop(L, 1);//pop the table off the stack
...
int32 field::announce_card(int16 step, uint8 playerid) {
	if(step == 0) {
		...
		if((player[playerid].isAI) && (core.duel_options & DUEL_SIMPLE_AI)) {
			...
			returns.at<int32>(0) = 86445415;//Red Gadget 
			auto message = pduel->new_message(MSG_HINT);
			message->write<uint8>(HINT_CODE);
			message->write<uint8>(playerid);
			message->write<uint64>(returns.at<int32>(0));
