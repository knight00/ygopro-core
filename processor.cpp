...
int32 field::process() {
	...
	case PROCESSOR_SELECT_FUSION: {
		if(it->step == 0) {
			...
			e.reason_effect = core.select_effects[returns.at<int32>(0)];
			e.reason_player = it->arg1;
			core.fusion_materials.clear();
			if(!core.select_effects[returns.at<int32>(0)]) {
				core.units.pop_front();
				return PROCESSOR_FLAG_CONTINUE;
			}
	...
	case PROCESSOR_DISCARD_HAND: {
		if(it->step == 0) {
			...
			message->write<uint8>(it->arg1);
			if(it->arg3 & REASON_DISCARD)
				message->write<uint64>(501);
			else
				message->write<uint64>(504);
	...
	case 0: {
		...
		if(core.select_chains.size() == 0) {
			...
			if(infos.phase == PHASE_DRAW)
				message->write<uint64>(20);
			else if(infos.phase == PHASE_STANDBY)
				message->write<uint64>(21);
			else if(infos.phase == PHASE_BATTLE_START)
				message->write<uint64>(28);
			else if(infos.phase == PHASE_BATTLE)
				message->write<uint64>(25);
			else
				message->write<uint64>(26);
...
int32 field::process_point_event(int16 step, int32 skip_trigger, int32 skip_freechain, int32 skip_new) {
	...
	case 31: {
		if(returns.at<int32>(0) == -1)
			return FALSE;
		const chain& newchain = core.select_chains[returns.at<int32>(0)];
		effect* peffect = newchain.triggering_effect;
		core.select_chains.clear();
		solve_continuous(peffect->get_handler_player(), peffect, nil_event);
...
int32 field::process_quick_effect(int16 step, int32 skip_freechain, uint8 priority) {
	...
	case 3: {
		if(core.select_chains.size() && returns.at<int32>(0) != -1) {
			chain newchain = core.select_chains[returns.at<int32>(0)];
			core.new_chains.push_back(newchain);
			effect* peffect = newchain.triggering_effect;
...
int32 field::process_idle_command(uint16 step) {
	...
	case 0: {
		...
		//////kdiy//////////////
		for(auto& pcard : player[infos.turn_player].list_szone) {
			if(pcard && ((pcard->is_position(POS_FACEUP | POS_FACEDOWN_ATTACK) && pcard->is_capable_change_position(infos.turn_player) && pcard->is_affected_by_effect(EFFECT_ORICA_SZONE))
		        || (pcard->is_position(POS_FACEDOWN) && pcard->is_can_be_flip_summoned(infos.turn_player))))
				core.repositionable_cards.push_back(pcard);
		}
		//////kdiy//////////////
	...
	case 1: {
		uint32 ctype = returns.at<int32>(0) & 0xffff;
		uint32 sel = returns.at<int32>(0) >> 16;
		if(ctype == 5) {
			...
			auto message = pduel->new_message(MSG_HINT);
			message->write<uint8>(HINT_EVENT);
			message->write<uint8>(1 - infos.turn_player);
			message->write<uint64>(23);
			core.select_chains.clear();
	...
	case 5: {
		card* target = core.summonable_cards[returns.at<int32>(0) >> 16];
	...
	case 6: {
		card* target = core.spsummonable_cards[returns.at<int32>(0) >> 16];
		core.summon_cancelable = TRUE;
	...
	case 7: {
		card* target = core.repositionable_cards[returns.at<int32>(0) >> 16];
	...
	case 8: {
		card* target = core.msetable_cards[returns.at<int32>(0) >> 16];
		core.summon_cancelable = TRUE;
	...
	case 9: {
		card* target = core.ssetable_cards[returns.at<int32>(0) >> 16];
	...
	case 13: {
		...
		if(returns.at<int32>(0) == POS_FACEUP_ATTACK)
			add_process(PROCESSOR_FLIP_SUMMON, 0, 0, (group*)target, target->current.controler, 0);
		else {
			core.phase_action = TRUE;
...
int32 field::process_battle_command(uint16 step) {
	...
	case 1: {
		int32 ctype = returns.at<int32>(0) & 0xffff;
		int32 sel = returns.at<int32>(0) >> 16;
		if(ctype == 0) {
			chain newchain = core.select_chains[sel];
			effect* peffect = newchain.triggering_effect;
			if(peffect->type & EFFECT_TYPE_CONTINUOUS) {
			...
			for(const auto& peff : eset) {
				if(peff->operation) {
					newchain.triggering_effect = peff;
	...
	case 26: {
		...
		if(bd[0]) {
			...
			if(indestructable_effect) {
				...
				message->write<uint8>(0);
				message->write<uint64>(indestructable_effect->owner->data.code);
				bd[0] = FALSE;
			} else
		...
		message->write(core.attacker->get_info_location());
		message->write<uint32>(aa);
		message->write<uint32>(ad);
		message->write<uint8>(bd[0]);
		if(core.attack_target) {
			message->write(core.attack_target->get_info_location());
			message->write<uint32>(da);
		...
		//////////kdiy/////////			
			...
			if(a > d) {
				...
				if(eset.size()) {
					...
					for(const auto& peff : eset) {
						if(peff->get_value() == static_cast<int32>(DOUBLE_DAMAGE))
							double_damage = true;
...
int32 field::process_turn(uint16 step, uint8 turn_player) {
	...
	case 12: {
		...
		core.skip_m2 = FALSE;
		if(returns.at<int32>(0) == 3 || core.force_turn_end) { // End Phase
			core.skip_m2 = TRUE;
		}
		//Main2
...
int32 field::add_chain(uint16 step) {
	...
	case 1: {
		...
		message->write<uint32>(phandler->data.code);
		message->write(phandler->get_info_location());
		message->write<uint8>(clit.triggering_controler);
		message->write<uint8>((uint8)clit.triggering_location);
	...
	case 4: {
		auto& clit = core.current_chain.back();
		chain& ch = core.select_chains[returns.at<int32>(0)];
		int32 playerid = clit.triggering_player;
		effect* peffect = ch.triggering_effect;
		card* phandler = peffect->get_handler();
		auto message = pduel->new_message(MSG_HINT);
	...
	case 1: {
		...
		if(cait->flag & CHAIN_DISABLE_ACTIVATE && is_chain_negatable(cait->chain_count)) {
			...
			if(peffect->is_flag(EFFECT_FLAG_COUNT_LIMIT) && (peffect->count_flag & EFFECT_COUNT_CODE_OATH)) {
				dec_effect_code(peffect->count_code, peffect->count_flag, cait->triggering_player);
			}
...
void field::refresh_location_info_instant() {
	...
	for(const auto& peff : eset) {
		uint32 p = peff->get_handler_player();
		uint32 value = peff->get_value();
...
int32 field::refresh_location_info(uint16 step) {
	...
	case 2: {
		returns.at<int32>(0) &= 0xff7fff7f;
		if(returns.at<int32>(0) == 0)
			returns.at<int32>(0) = 0x80;
		if(core.units.begin()->peffect->get_handler_player() == 0) {
			core.units.begin()->peffect->value = returns.at<int32>(0);
			player[0].disabled_location |= returns.at<int32>(0) & 0xff7f;
			player[1].disabled_location |= (returns.at<int32>(0) >> 16) & 0xff7f;
		} else {
			core.units.begin()->peffect->value = ((returns.at<int32>(0) << 16) | (returns.at<int32>(0) >> 16));
			player[1].disabled_location |= returns.at<int32>(0) & 0xff7f;
			player[0].disabled_location |= (returns.at<int32>(0) >> 16) & 0xff7f;
		}
		core.units.begin()->step = 0;
		return FALSE;
	}
