...
int32 scriptlib::effect_get_description(lua_State *L) {
	...
	effect* peffect = *(effect**) lua_touserdata(L, 1);
	if (peffect) {
		lua_pushnumber(L, peffect->description);
		return 1;
	}
	return 0;
}
...
int32 scriptlib::effect_has_property(lua_State *L) {
	check_param_count(L, 2);
	check_param(L, PARAM_TYPE_EFFECT, 1);
	effect* peffect = *(effect**) lua_touserdata(L, 1);
...
int32 scriptlib::effect_has_category(lua_State *L) {
	check_param_count(L, 2);
	check_param(L, PARAM_TYPE_EFFECT, 1);
	effect* peffect = *(effect**) lua_touserdata(L, 1);
...
int32 scriptlib::effect_has_type(lua_State *L) {
	check_param_count(L, 2);
	check_param(L, PARAM_TYPE_EFFECT, 1);
	effect* peffect = *(effect**) lua_touserdata(L, 1);
...
int32 scriptlib::effect_can_be_activated(lua_State *L) {
	check_param_count(L, 2);
	check_param(L, PARAM_TYPE_EFFECT, 1);
	uint32 playerid = lua_tointeger(L, 2);
	effect* peffect = *(effect**) lua_touserdata(L, 1);
