...
int32 scriptlib::effect_set_description(lua_State *L) {
	...
	check_param(L, PARAM_TYPE_EFFECT, 1);
	effect* peffect = *(effect**)lua_touserdata(L, 1);
	uint64 v = lua_casted_decl(L,  2, v);
...
int32 scriptlib::effect_set_code(lua_State *L) {
	...
	check_param(L, PARAM_TYPE_EFFECT, 1);
	effect* peffect = *(effect**) lua_touserdata(L, 1);
	uint32 v = lua_casted_decl(L,  2, v);
	peffect->code = v;
	return 0;
}
int32 scriptlib::effect_set_range(lua_State *L) {
	...
	check_param(L, PARAM_TYPE_EFFECT, 1);
	effect* peffect = *(effect**) lua_touserdata(L, 1);
	uint16 v = lua_casted_decl(L,  2, v);
	peffect->range = v;
	return 0;
}
int32 scriptlib::effect_set_target_range(lua_State *L) {
	...
	check_param(L, PARAM_TYPE_EFFECT, 1);
	effect* peffect = *(effect**) lua_touserdata(L, 1);
	uint16 s = lua_casted_decl(L,  2, s);
	uint16 o = lua_casted_decl(L,  3, o);
	peffect->s_range = s;
	peffect->o_range = o;
...
int32 scriptlib::effect_set_absolute_range(lua_State *L) {
	...
	check_param(L, PARAM_TYPE_EFFECT, 1);
	effect* peffect = *(effect**) lua_touserdata(L, 1);
	uint32 playerid = lua_casted_decl(L,  2, playerid);
	uint16 s = lua_casted_decl(L,  3, s);
	uint16 o = lua_casted_decl(L,  4, o);
	if(playerid == 0) {
		peffect->s_range = s;
		peffect->o_range = o;
	} else {
...
int32 scriptlib::effect_set_count_limit(lua_State *L) {
	...
	check_param(L, PARAM_TYPE_EFFECT, 1);
	effect* peffect = *(effect**) lua_touserdata(L, 1);
	uint8 v = lua_casted_decl(L,  2, v);
	uint32 code = 0;
	if(lua_gettop(L) >= 3)
		code = lua_casted_decl(L,  3, code);
	int32 flag = 0;
	if(lua_gettop(L) >= 4)
		flag = lua_casted_decl(L,  4, flag);
	else {
...
int32 scriptlib::effect_set_reset(lua_State *L) {
	...
	effect* peffect = *(effect**) lua_touserdata(L, 1);
	uint32 v = lua_casted_decl(L,  2, v);
	uint16 c = lua_casted_decl(L,  3, c);
	if(c == 0)
		c = 1;
...
int32 scriptlib::effect_set_property(lua_State *L) {
	...
	check_param(L, PARAM_TYPE_EFFECT, 1);
	effect* peffect = *(effect**) lua_touserdata(L, 1);
	uint32 v1 = lua_casted_decl(L,  2, v1);
	uint32 v2 = lua_casted_decl(L,  3, v2);
...
int32 scriptlib::effect_set_label(lua_State *L) {
	...
	for(int32 i = 2; i <= lua_gettop(L); ++i)
		peffect->label.push_back(lua_casted(L, i, uint32));
	return 0;
}
...
int32 scriptlib::effect_set_hint_timing(lua_State *L) {
	...
	check_param(L, PARAM_TYPE_EFFECT, 1);
	effect* peffect = *(effect**) lua_touserdata(L, 1);
	uint32 vs = lua_casted_decl(L,  2, vs);
	uint32 vo = vs;
	if(lua_gettop(L) >= 3)
		vo = lua_casted_decl(L,  3, vo);
...
int32 scriptlib::effect_set_value(lua_State *L) {
	...
	if (lua_isfunction(L, 2)) {
		...
		if(lua_isboolean(L, 2))
			...
			peffect->value = lua_casted_decl(L,  2, peffect->value);
		else
			peffect->value = static_cast<int32>(std::round(lua_tonumber(L, 2)));
	}
	return 0;
}
...
int32 scriptlib::effect_get_description(lua_State *L) {
	...
	effect* peffect = *(effect**) lua_touserdata(L, 1);
	if (peffect) {
		lua_pushnumber(L, static_cast<lua_Number>(peffect->description));
		return 1;
	}
	return 0;
}
...
int32 scriptlib::effect_is_has_property(lua_State *L) {
	...
	check_param(L, PARAM_TYPE_EFFECT, 1);
	effect* peffect = *(effect**) lua_touserdata(L, 1);
	uint32 tflag1 = lua_casted_decl(L,  2, tflag1);
	uint32 tflag2 = lua_casted_decl(L,  3, tflag2);
...
int32 scriptlib::effect_is_has_category(lua_State *L) {
	...
	check_param(L, PARAM_TYPE_EFFECT, 1);
	effect* peffect = *(effect**) lua_touserdata(L, 1);
	uint32 tcate = lua_casted_decl(L,  2, tcate);
